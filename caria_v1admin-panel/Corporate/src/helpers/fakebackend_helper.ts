...
// Ecommerce
// get Products
export const getProducts = () => api.get(url.GET_PRODUCTS).then(resp => {
  const data = resp.data || resp;
  return (data || []).map((p: any) => ({
    ...p,
    name: p.title || p.name,
    category: p.type || p.category,
    stock: p.status || p.stock || "Yayınlandı",
    orders: p.orders || 0,
    rating: p.rating || 4.5,
    publishedDate: p.publishedDate || "15 Jan, 2026",
    price: p.price || 0,
    image: p.image || ""
  }));
});
...
// get Leads
export const getLeads = () => api.get(url.GET_LEADS).then(resp => {
  const data = resp.data || resp;
  return (data || []).map((l: any) => ({
    ...l,
    name: l.name,
    company: l.message ? (l.message.length > 30 ? l.message.substring(0, 30) + "..." : l.message) : (l.property_id ? `İlan ID: ${l.property_id}` : "İlan Talebi"),
    score: "5.0",
    phone: l.phone,
    location: l.email,
    date: l.created_at || "15 Jan, 2026",
    tags: ["İnternet", "Yeni"],
